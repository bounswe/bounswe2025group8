...
const TaskDetailComponent = () => {
  ...
  // Set completed date in task data when status changes to completed
  React.useEffect(() => {
    if (status === "completed" && taskData && !taskData.completedDate) {
      ...
      // For now, we'll just modify the local state
      if (task) {
        dispatch(setTask(updatedTaskData));
      }
    }
  }, [status, taskData, task, dispatch]);
  ...
  if (status === "completed") {
    return (
      <>
        <TaskDetailCompleted task={taskData} />
        <Snackbar
          open={snackbarOpen}
