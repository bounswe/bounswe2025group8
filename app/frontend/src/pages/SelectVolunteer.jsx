...
import { getTaskById, getTaskVolunteers, assignVolunteers, getMockTaskById, getMockTaskVolunteers } from '../features/request/services/requestService';
...
const SelectVolunteer = () => {
        ...
        try {
          ...
          if (volunteersData && Array.isArray(volunteersData)) {
            volunteersData = volunteersData.map(volunteer => ({
              ...
              reviewCount: volunteer.user?.completed_task_count || volunteer.completedTasks || 0,
              avatar: volunteer.user?.avatar || volunteer.avatar || `https://ui-avatars.com/api/?name=${volunteer.user?.name || volunteer.name}&background=random`,
              skills: volunteer.skills || ['Volunteering'],
              completedTasks: volunteer.user?.completed_task_count || volunteer.completedTasks || 0,
              user: volunteer.user || volunteer,
              volunteerRecord: volunteer
            }));
          }
        } catch (volunteersError) {
          console.warn('Volunteers API failed, using mock data:', volunteersError);
  ...
  // Permission check
  const isTaskCreator = currentUser && task && currentUser.id === task.creator?.id;
  ...
  const handleSelectVolunteers = async () => {
    ...
    try {
      ...
      alert(`Successfully assigned ${selectedVolunteers.length} volunteer(s) to this task`);
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#f5f5f5' }}>
      ...
      <Box sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Box sx={{ mb: 3, p: 2, bgcolor: 'white', borderRadius: 2, boxShadow: 1 }}>
          <Typography variant="body1" color="text.secondary">
            <strong>{volunteers.length}</strong> people have volunteered for this task
          </Typography>
          <Typography variant="body1" color="text.secondary">
            Select up to <strong>{maxVolunteers}</strong> volunteer{maxVolunteers > 1 ? 's' : ''} to assign
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Currently selected: <strong>{selectedVolunteers.length}</strong> of {maxVolunteers}
          </Typography>
        </Box>
        ...
        {volunteers.length === 0 ? (
          <Box sx={{ 
            ...
            <Typography variant="body2" color="text.secondary">
              Wait for people to volunteer for this task before you can select them.
            </Typography>
          </Box>
        ) : (
          <Grid container spacing={2} sx={{ mb: 3 }}>
            ...
            const canSelect = selectedVolunteers.length < maxVolunteers || isSelected;
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={volunteer.id}>
                <Card 
                  ...
                  onClick={() => canSelect && handleVolunteerSelect(volunteer.id)}
                >
                    ...
                    <Box sx={{ position: 'relative', mb: 2 }}>
                      <Avatar 
                        ...
                        sx={{ 
                          ...
                          height: 80, 
                          mx: 'auto',
                          border: isSelected ? '3px solid #7c4dff' : '3px solid transparent'
                        }}
                      />
                      {/* Selection Circle */}
                      <Box
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
          ...
          >
            Select {selectedVolunteers.length > 0 ? `(${selectedVolunteers.length})` : ''}
          </Button>
        </Box>
      </Box>
    </Box>
